"""
Ultra Trading Robot - Geçmişe Yönelik Backtest
...
class UltraRobotBacktest:
    """Ultra Trading Robot Backtest Engine"""
    ...
    def run_historical_backtest(self, symbol: str, start_date: str, end_date: str, 
        ...
        try:
            ...
            # Geçmiş veri çek
            historical_data = self._get_historical_data(symbol, start_date, end_date, timeframes)
    ...
    def _get_historical_data(self, symbol: str, start_date: str, end_date: str, 
        ...
        try:
            ...
            for timeframe in timeframes:
                ...
                # Veri çek
                df = stock.history(period=period, interval=timeframe.value, start=start_date, end=end_date)
    ...
    def _generate_historical_signals(self, data: pd.DataFrame, indicators: Dict, timeframe: TimeFrame) -> List[Dict]:
        ...
        try:
            ...
            # Her gün için sinyal kontrol et
            for i in range(50, len(data)):  # İlk 50 günü atla (indikatör hesaplama için)
    ...
    def generate_report(self, symbol: str) -> str:
        ...
        try:
            ...
            report = f"""
# 🚀 ULTRA TRADING ROBOT - PERFORMANS RAPORU
## 📊 {symbol} Analizi
...
if __name__ == "__main__":
    ...
    logger.info("🚀 Ultra Robot Backtest başlatılıyor...")
    ...
    if "error" not in comparison:
        ...
        # Sonuçları kaydet
        with open(f"backtest_report_{test_symbol}.txt", "w", encoding="utf-8") as f:
            f.write(report)
        
        logger.info(f"📄 Rapor kaydedildi: backtest_report_{test_symbol}.txt")
    else:
